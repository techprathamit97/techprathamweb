...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    console.log('=== AUTO PROCESSING BBB RECORDINGS ===');

    await connectMongo();

    // Find all completed classes that might have recordings but haven't been processed
    const completedClasses = await ModuleClass.find({
      $or: [
        { status: 'completed' },
        { status: 'live', actualEndTime: { $ne: null } }
      ],
      bbbMeetingId: { $exists: true, $ne: null },
      bbbRecordingProcessed: { $ne: true }
    });

    console.log(`Found ${completedClasses.length} classes to check for recordings`);

    if (completedClasses.length === 0) {
      return res.status(200).json({
        success: true,
        message: 'No classes found that need recording processing',
        processedClasses: 0
      });
    }
    ...
    for (const moduleClass of completedClasses) {
      try {
        const searchMeetingId = moduleClass.bbbMeetingId || `class-${moduleClass._id}`;
        
        console.log(`Checking recordings for class: ${moduleClass.moduleTitle} (${searchMeetingId})`);
        ...
        console.log(`BBB recordings response for ${searchMeetingId}:`, recordingsXML.substring(0, 200));
        ...
        if (recordingsXML.includes('<returncode>SUCCESS</returncode>')) {
          ...
          if (recordingMatches && recordingMatches.length > 0) {
            console.log(`Found ${recordingMatches.length} recording(s) for ${searchMeetingId}`);
        ...
        // Mark class as processed even if no recordings were found (to avoid checking again)
        moduleClass.bbbRecordingProcessed = true;
        moduleClass.bbbRecordingProcessedAt = new Date();

        // Update class status to completed if it was live
        if (moduleClass.status === 'live' && moduleClass.actualEndTime) {
        ...
        if (recordingsProcessed > 0) {
          totalProcessed += recordingsProcessed;
          console.log(`Updated class ${moduleClass.moduleTitle} with ${recordingsProcessed} new recording(s)`);
        }

      } catch (classError: any) {
        console.error(`Error processing class ${moduleClass._id}:`, classError);
        processResults.push({
          classId: moduleClass._id,
          className: moduleClass.moduleTitle,
    ...
    return res.status(200).json({
      success: true,
      message: `Auto-processing completed. ${totalProcessed} recordings processed across ${completedClasses.length} classes.`,
      totalRecordingsProcessed: totalProcessed,
      classesChecked: completedClasses.length,
      processResults: processResults
    });

  } catch (error: any) {
    console.error('Error in auto-processing BBB recordings:', error);
    return res.status(500).json({
      success: false,
      error: 'Failed to auto-process BBB recordings: ' + error.message
    });
  }
}